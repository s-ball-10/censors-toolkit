# Project Page for the ICML 2026 Oral Position Paper "The Alignment Community is Unintentionally Building a Censor’s Toolkit"
by Sarah Ball and Phil Hackemann

🔗 **[Visit the Project Page](https://s-ball-10.github.io/censors-toolkit/)**

---

## 📝 Abstract
Our position paper argues that modern AI alignment methods – originally designed to prevent harmful output – are dual-use technologies that may easily be misused by malicious actors for censorship and manipulation. By mapping current alignment techniques to the possibility and actual cases of misuse, we show that the quest for a “perfectly aligned” model inadvertently also provides malicious actors with an ever-improving tool for informational dominance. We need to discuss this dual-use potential now, as its risk is exacerbated by rapid user adoption of AI as information provider, economic power asymmetries, and a political landscape that increasingly shifts towards authoritarianism. We conclude by urging the community to consider the intentional misuse of AI alignment mechanisms and propose mitigation strategies to safeguard against this dual-use potential.

⬇️ **[Read the full paper PDF](https://s-ball-10.github.io/censors-toolkit/assets/The%20Alignment%20Community%20is%20Unintentionally%20Building%20a%20Censors%20Toolkit.pdf)**

---

## 🎓 Citation

If you find our paper useful for your research, please consider citing our work:

```bibtex
@inproceedings{ballhackemann2026alignment,
  title     = {Position: The Alignment Community is Unintentionally Building a Censor's Toolkit},
  author    = {Ball, Sarah and Hackemann, Phil},
  booktitle = {Proceedings of the 43rd International Conference on Machine Learning},
  series    = {Proceedings of Machine Learning Research},
  year      = {2026}
}
```

---

## 🌐 About this page
This is the documentation file of the official project webpage for our position paper, written in HTML + CSS + JS, hosted at GitHub Pages. The main file is index.html. You can open it via the link above.

♿ The website is fully accessible to screenreaders and also visible without JavaScript enabled.

---

## © Copyright
© 2026 Sarah Ball & Phil Hackemann, all rights reserved. Our paper is licensed under CC-BY.
